...
async def start(message: types.Message):
    start_message = "Привет! Я бот для автопостинга с чужих каналов в ваш личный. Чтобы увидеть список команд, введите /help\n\n"
...
async def add_channel(message: types.Message):
    try:
        ...
        save_channels('channels.pickle', channels)
...
async def add_destination_channel(message: types.Message):
    try:
        ...
        save_channels('destination_channels.pickle', destination_channels)
...
async def set_channel_mapping(message: types.Message):
    ...
    try:
        ...
        save_channels('channel_mapping.pickle', channel_mapping)
    ...
    for destination_channel_id in destination_channels:
        for message_group in grouped_messages.values():
            if len(message_group) > 1 and message_group[0].grouped_id:
                ...
                caption = "\n".join([replace_link(replace_at_word(
                    msg.text, "@test"), new_link) for msg in message_group if msg.text])
                await client.send_file(destination_channel_id, media_list, caption=caption)
            else:
                if message_group[0].media:
                    updated_text = replace_link(replace_at_word(
                        message_group[0].text, "@test"), new_link)
